...
class WorkoutPlannerApp(App):
    ...
    def generate_workout_plan(self, instance):
        ...
        try:
            ...
            def determine_reps_weights_rest(training_goal, weight_for_10_rep, base_rest_time):
                ...
                if training_goal == 'Strength':
                    ...
                    for i in range(30):  # I used 30 here as per your original code
                        ...
                        reps = max(round(reps * 0.8), 1) # Decreasing the number of reps by 20%
            ...
            input_id_counter = 0  # Initialize a counter for input IDs
            # Display the generated plan
            for exercise_row in selected_exercises:
                exercise_name = exercise_row['Name']
                num_sets = exercise_row['sets']
                time_per_set = exercise_row['time_per_set']
                rest_time = exercise_row['rest_time']

                # Display exercise name
                exercise_box = BoxLayout(size_hint_y=None, height=30)
                exercise_box.add_widget(Label(text=exercise_name, size_hint_x=0.5))
                self.output_grid.add_widget(exercise_box)

                # Display each set and rest time
                for set_num in range(1, num_sets + 1):
                    set_box = BoxLayout(size_hint_y=None, height=30)
                    
                    # Add TextInput for editable fields
                    reps_input = TextInput(text="10", size_hint_x=0.2)
                    weight_input = TextInput(text="50", size_hint_x=0.2)
                    time_input = TextInput(text=str(time_per_set), size_hint_x=0.2)
                    
                    set_box.add_widget(Label(text=f"Set {set_num}", size_hint_x=0.2))
                    ...
                    # Add Label to display rest time (if not the last set)
                    if set_num < num_sets:
                        rest_box = BoxLayout(size_hint_y=None, height=30)
                        rest_box.add_widget(Label(text=f"Rest: {rest_time} sec", size_hint_x=0.2))
