...
class TrainingSessionApp(App):
    ...
    def submit_initial_input(self, instance):
        ...
        prompt = f"Generate a well-structured JSON representation for a {self.num_exercises}-exercise {self.muscle_focus} training session. The session title should be {self.num_exercises} exercises, {self.duration} minutes long, focusing on {self.muscle_focus}. Total reps: . For each exercise, include the following details: exercise name, number of reps, duration, explanation of how to perform it, and the materials needed. Please write the JSON content between [start] and [end] to make it easily parseable. Refer to the example below for the expected format:\n[start]{{\nname: Triceps Workout,\nexercises: [{{name: Tricep Dips, reps: 12, time: 0, explanation: To perform tricep dips, find parallel bars or use the edge of a sturdy surface. Lower your body by bending your elbows until they are at a 90-degree angle. Push yourself back up using your triceps. This exercise targets your triceps., material: Parallel Bars or Stable Surface, sets: 1/2 }},{{ name: rest, reps: 0, time: 60, explanation: Rest for 60 seconds., material: None (Bodyweight) }},{{ name: Tricep Dips, reps: 12, time: 0, explanation: To perform tricep dips, find parallel bars or use the edge of a sturdy surface. Lower your body by bending your elbows until they are at a 90-degree angle. Push yourself back up using your triceps. This exercise targets your triceps., material: Parallel Bars or Stable Surface, sets: 2/2 }},{{ name: rest, reps: 0, time: 60, explanation: Rest for 60 seconds., material: None (Bodyweight) }},{{ name: Diamond Push-Ups, reps: 10, time: 0, explanation: For diamond push-ups, get into a push-up position, but bring your hands close together under your chest so your thumbs and index fingers form a diamond shape. Lower your body by bending your elbows and then push back up. This exercise emphasizes the triceps., material: None (Bodyweight), sets: 1/3 }},{{ name: rest, reps: 0, time: 60, explanation: Rest for 60 seconds between sets., material: None (Bodyweight) }},{{ name: Diamond Push-Ups, reps: 10, time: 0, explanation: For diamond push-ups, get into a push-up position, but bring your hands close together under your chest so your thumbs and index fingers form a diamond shape. Lower your body by bending your elbows and then push back up. This exercise emphasizes the triceps., material: None (Bodyweight), sets: 2/3 }},{{ name: rest, reps: 0, time: 60, explanation: Rest for 60 seconds between sets., material: None (Bodyweight) }},{{ name: Diamond Push-Ups, reps: 10, time: 0, explanation: For diamond push-ups, get into a push-up position, but bring your hands close together under your chest so your thumbs and index fingers form a diamond shape. Lower your body by bending your elbows and then push back up. This exercise emphasizes the triceps., material: None (Bodyweight), sets: 3/3 }},{{ name: rest, reps: 0, time: 60, explanation: Rest for 60 seconds., material: None (Bodyweight) }},{{ name: Tricep Kickbacks, reps: 15, time: 0, explanation: Hold a dumbbell in each hand and bend your knees slightly. Hinge at your hips and lean forward. Extend your arms straight back, squeezing your triceps at the top of the movement. Lower the dumbbells and repeat. This exercise isolates the triceps., material: Dumbbells, sets: 1/2 }},{{ name: rest, reps: 0, time: 60, explanation: Rest for 60 seconds., material: None (Bodyweight) }},{{ name: Tricep Kickbacks, reps: 15, time: 0, explanation: Hold a dumbbell in each hand and bend your knees slightly. Hinge at your hips and lean forward. Extend your arms straight back, squeezing your triceps at the top of the movement. Lower the dumbbells and repeat. This exercise isolates the triceps., material: Dumbbells, sets: 2/2 }}]\n}}[end]\n\nPlease ensure the JSON adheres to this structure and contains all the necessary details for each exercise. Remember than names should be written between quotes."
        ...
        corrected_json_text = self.correct_json_format(extracted_text)
        
        # Read existing content from JSON file if it exists
        try:
            with open('training_session.json', 'r') as f:
        ...
        # Append the new session to the existing content
        new_session = json.loads(corrected_json_text)
        ...
        # Update the training sessions based on the saved JSON
        self.training_sessions = existing_sessions

        self.screen_manager.current = 'session_list'

    def correct_json_format(self, malformed_json_text):
        prompt = f"Answer with the new json file only, do not write anything else. Please correct the following JSON content to ensure it's properly formatted, and includes the correct commas, parenthesis and brackets:\n{malformed_json_text}"
        ...
        corrected_json_text = response.choices[0].text.strip()
