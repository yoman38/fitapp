...
class WorkoutPlannerApp(App):
    ...
    def generate_workout_plan(self, instance):
        ...
        try:
            ...
            def calculate_time(self, exercise, num_sets, user_rest_time):
                est_time_for_10_reps = exercise['Est_Time_for_10_Reps']
                coeff_rest_time = exercise['coeff_rest_time']
                
                # Calculate time for one set
                time_per_set = (est_time_for_10_reps / 10)
                
                # Calculate rest time based on user input and coeff_rest_time
                rest_time = user_rest_time * coeff_rest_time
                
                # Total time for this exercise with num_sets
                total_time = num_sets * (time_per_set + rest_time)
            ...
            def determine_reps_weights_rest(training_goal, base_reps, base_weight, base_rest_time):
                ...
                if training_goal == 'Strength':
                    reps = base_reps
                    for i in range(5):  # I used 30 here as per your original code
            ...
            # Create a list of muscle names
            muscle_list = list(self.muscle_sliders.keys())

            # Step 1: Initial Exercise Selection
            initial_exercise_idx = random.randint(0, len(filtered_exercises_list) - 1)
            initial_exercise = filtered_exercises_list[initial_exercise_idx]
            initial_vector = np.array([initial_exercise[muscle] for muscle in muscle_list])

            # Assume 3 sets for the initial exercise
            initial_ponderation_factor = 3
            final_vector = initial_vector * initial_ponderation_factor
            total_ponderation_factor = initial_ponderation_factor

            # Calculate initial exercise time
            initial_exercise_time = calculate_time(initial_exercise, 3)  # Assuming a function that calculates time

            # Initialize other variables
            selected_exercises = [initial_exercise_idx]
            remaining_time_budget_seconds = (total_training_length_minutes * 60) - initial_exercise_time
            exercise_plan_output = []
            # Initialize a dictionary to keep track of the number of sets for each selected exercise
            exercise_sets_dict = {}
            ...
            while len(selected_exercises) < desired_num_exercises:
                ...
                for idx, exercise in enumerate(filtered_exercises_list):
                    if idx in selected_exercises:
                        continue  # Skip already selected exercises
                    
                    for num_sets in range(1, 6):  # Consider 1 to 5 sets
                        exercise_vector = np.array([exercise[muscle] for muscle in muscle_list])
                        ponderation_factor = num_sets  # Number of sets used as the ponderation factor
                        ...
                        if distance < best_next_distance:
                            ...
                            best_next_time = calculate_time(exercise, num_sets, user_rest_time)
                            best_next_sets = num_sets
                
                selected_exercises.append(best_next_exercise_idx)
                final_vector = best_next_vector
                total_ponderation_factor += best_next_sets
                total_workout_time_seconds += best_next_time  # Update the total workout time
                exercise_sets_dict[best_next_exercise_idx] = best_next_sets  # Update the number of sets for this exercise

            # Step 3: Time Adjustment
            desired_total_time = total_training_length_minutes * 60

            while total_workout_time_seconds < 0.85 * desired_total_time or total_workout_time_seconds > 1.15 * desired_total_time:
                if total_workout_time_seconds > desired_total_time:
                    # Find the exercise with the most sets
                    max_sets_exercise = max(exercise_sets_dict, key=exercise_sets_dict.get)
                    
                    if exercise_sets_dict[max_sets_exercise] > 1:
                        # Reduce by one set and update the total workout time
                        exercise_sets_dict[max_sets_exercise] -= 1
                        total_workout_time_seconds -= calculate_time(filtered_exercises_list[max_sets_exercise], 1, user_rest_time)
                else:
                    # Find the exercise with the least sets
                    min_sets_exercise = min(exercise_sets_dict, key=exercise_sets_dict.get)
                    
                    # Increase by one set and update the total workout time
                    exercise_sets_dict[min_sets_exercise] += 1
                    total_workout_time_seconds += calculate_time(filtered_exercises_list[min_sets_exercise], 1, user_rest_time)
            ...
            for idx in selected_exercises:
                ...
                # Use the updated number of sets from the time adjustment
                num_sets = exercise_sets_dict[idx]
