...
# Read CSV file and store in exercise_data_list
exercise_data_list = []
with open("C:\\Users\\gbray\\Documents\\fitapp\\exercise_data2.csv", "r") as csvfile:  # <-- Replace the path with the actual path to the CSV file on your machine
    csvreader = csv.DictReader(csvfile)
    for row in csvreader:
        # Convert muscle group values to float
        for muscle in ['Lats', 'Chest', 'Deltoids', 'Biceps', 'Triceps', 'Abs', 'Forearm', 'Quads', 'Hams', 'Calfs', 'Glutes', 'Lumbar', 'Trapezius']:
            row[muscle] = float(row[muscle])
        exercise_data_list.append(row)
...
class WorkoutPlannerApp(App):
    ...
    def generate_workout_plan(self, instance):
        ...
        try:
            ...
            exercise_data2_list = exercise_data_list
